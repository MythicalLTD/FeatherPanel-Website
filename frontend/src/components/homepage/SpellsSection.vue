<template>
    <section id="spells" class="relative z-10 py-20 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto">
            <div class="text-center mb-16">
                ...
                <p class="text-xl text-gray-400 animate-slide-in-up stagger-1">
                    Discover game server configurations (eggs) for all your favorite games
                </p>
            </div>
            ...
            <div class="max-w-2xl mx-auto mb-12 animate-slide-in-up stagger-2">
                <div class="relative">
                    ...
                    <input
                        v-model="searchQuery"
                        type="text"
                        placeholder="Search spells (e.g., Minecraft, CS2, Bungeecord)..."
                        class="w-full pl-12 pr-4 py-4 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent backdrop-blur-sm transition-all duration-300"
                        @input="handleSearch"
                    />
                    <button
                        v-if="searchQuery"
            ...
            <div v-else-if="error" class="text-center py-20">
                ...
                <p class="text-xl text-red-400">Failed to load spells</p>
                <p class="text-gray-400 mt-2">{{ error }}</p>
            </div>

            <!-- No Results -->
            <div
                v-else-if="spells.length === 0 && !loading"
                class="text-center py-20"
            >
                <Wand2 class="w-16 h-16 text-gray-400 mx-auto mb-4" />
                ...
                >
                    ...
                    <div class="flex items-start gap-4 mb-4">
                        ...
                        >
                            ...
                            />
                            <Wand2 v-else class="w-8 h-8 text-purple-400" />
                        </div>
                    ...
                    <!-- Spell Tags -->
                    <div v-if="spell.tags && spell.tags.length > 0" class="flex flex-wrap gap-2 mb-4">
                        <span
                    ...
                    <div class="flex items-center justify-between text-sm border-t border-white/10 pt-4">
                        <div class="flex items-center gap-4">
                            <div class="flex items-center gap-1 text-gray-400">
                                ...
                                <span>{{ formatDownloads(spell.downloads) }}</span>
                            </div>
                            <div v-if="spell.latest_version?.version" class="flex items-center gap-1 text-gray-400">
                                <Tag class="w-4 h-4" />
                                <span>{{ spell.latest_version.version }}</span>
                            </div>
                        </div>
                        <a
                            v-if="spell.website"
            ...
            <div class="mt-12 text-center text-sm text-gray-400">
                <p v-if="pagination">
                    Showing {{ pagination.from }}-{{ pagination.to }} of {{ pagination.total_records }} spells
                </p>
            </div>
        </div>
    </section>
...
const fetchSpells = async (page = 1, query = '') => {
    try {
        loading.value = true;
        error.value = null;
        ...
        const url = `/api/online-spells-list${params.toString() ? '?' + params.toString() : ''}`;
        const response = await fetch(url);
        const result = await response.json();

        if (result.success && result.data) {
            spells.value = result.data.spells || [];
            pagination.value = result.data.pagination || null;
            currentPage.value = page;
        } else {
            error.value = result.error_message || 'Failed to fetch spells';
        }
    } catch (err) {
        error.value = err.message || 'An error occurred while fetching spells';
...
const handleSearch = () => {
    ...
    searchTimeout.value = setTimeout(() => {
        currentPage.value = 1;
        fetchSpells(1, searchQuery.value);
    }, 500);
};
...
const goToPage = (page) => {
    ...
    fetchSpells(page, searchQuery.value);
    // Scroll to top of section
    document.getElementById('spells')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
};
